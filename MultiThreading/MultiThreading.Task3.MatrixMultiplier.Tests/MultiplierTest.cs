...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            IMatricesMultiplier regularMatricesMultiplier = new MatricesMultiplier();
            IMatricesMultiplier parallelMatricesMultiplier = new MatricesMultiplierParallel();

            Matrix m1 = new Matrix(matrixSize, matrixSize, true);
            Matrix m2 = new Matrix(matrixSize, matrixSize, true);

            long sequentialTime = MeasureTime(regularMatricesMultiplier);
            long parallelTime = MeasureTime(parallelMatricesMultiplier);

            Assert.IsTrue(
                parallelTime < sequentialTime, 
                $"The sequential time: {sequentialTime}ms\nTheParallel time: {parallelTime}ms");

            long MeasureTime(IMatricesMultiplier matricesMultiplier)
            {
